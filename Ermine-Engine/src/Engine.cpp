...
bool engine::Init(GLFWwindow* windowContext)
{
	...
	// Add a light entities
	auto redLightEntity = ECS::GetInstance().CreateEntity();
	ECS::GetInstance().AddComponent(redLightEntity, Transform(Vec3(1, 2, 0), Vec3(1, 1, 1), Vec3(0, 0, 0)));
	...
	ECS::GetInstance().AddComponent(blueLightEntity, Transform(Vec3(-1, 2, 0), Vec3(1, 1, 1), Vec3(0, 0, 0)));
	ECS::GetInstance().AddComponent(blueLightEntity, ObjectMetaData("LightBlue", "Light", true));
	ECS::GetInstance().AddComponent(blueLightEntity, Light(Vec3(0, 1, 0), 1.0f, LightType::POINT));
	

	auto greenLightEntity = ECS::GetInstance().CreateEntity();
	ECS::GetInstance().AddComponent(greenLightEntity, Transform(Vec3(0, 2, -3), Vec3(1, 1, 1), Vec3(0, 0, 0)));
	ECS::GetInstance().AddComponent(greenLightEntity, ObjectMetaData("LightGreen", "Light", true));
	ECS::GetInstance().AddComponent(greenLightEntity, Light(Vec3(0, 0, 1), 1.0f, LightType::POINT));
