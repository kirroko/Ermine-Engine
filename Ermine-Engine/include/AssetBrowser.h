...
namespace Ermine {
    namespace ImguiUI {
        ...
        struct ExampleAssetB
        {
            ImGuiID ID;
            int     Type;

            ExampleAssetB(ImGuiID id, int type) { ID = id; Type = type; }

            static const ImGuiTableSortSpecs* current_sortSpecs;

            static int IMGUI_CDECL CompareWithSortSpecs(const void* lhs, const void* rhs);
            static void SortWithSortSpecs(ImGuiTableSortSpecs* sort_specs, ExampleAssetB* items, int items_count);
        };
        ...
        struct ExampleAssetsBrowser
        {
            ...
            // State
            ImVector<ExampleAssetB> Items;               // Our items
            ...
            void AddItems(int count)
            {
                ...
                for (int n = 0; n < count; n++, NextItemId++)
                    Items.push_back(ExampleAssetB(NextItemId, (NextItemId % 20) < 15 ? 0 : (NextItemId % 20) < 18 ? 1 : 2));
                RequestSort = true;
            }
            void ClearItems()
            {
                ...
                if (AllowSorting)
                {
                    ...
                    if (ImGui::BeginTable("for_sort_specs_only", 2, table_flags_for_sort_specs, ImVec2(0.0f, ImGui::GetFrameHeight())))
                    {
                        ...
                        if (ImGuiTableSortSpecs* sort_specs = ImGui::TableGetSortSpecs())
                            ...
                            {
                                ExampleAssetB::SortWithSortSpecs(sort_specs, Items.Data, Items.Size);
                ...
                if (ImGui::BeginChild("Assets", ImVec2(0.0f, -ImGui::GetTextLineHeightWithSpacing()), ImGuiChildFlags_Borders, ImGuiWindowFlags_NoMove))
                {
                    ...
                    while (clipper.Step())
                    {
                        for (int line_idx = clipper.DisplayStart; line_idx < clipper.DisplayEnd; line_idx++)
                        {
                            ...
                            {
                                ExampleAssetB* item_data = &Items[item_idx];
